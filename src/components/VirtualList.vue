<template>
  <div class="vtlist" ref="listElRef" :style="listStyle" @scroll.passive="onscroll">
    <VirtualListTable class="vtlist-inner" ref="listInnerRef" :style="listInnerStyle" >
      <template #prepend>
...
const cpt = defineComponent({
  ...
  props: {
    ...
    disabled: Boolean, // 是否可拖动
    horizontal: Boolean, // 是否垂直
  ...
  setup(props) {
    ...
    const listElRef = ref<HTMLElement>(); // 外层div
    ...
    function onscroll() {
      ...
      const currentScroll = getScroll(listEl); // scrollTop
      if (
        prevScroll != null &&
    ...
    async function update() {
      ...
      if (!avgSize.value) {
        avgSize.value = getAvgSize(); // 默认取10个获取平均高度
      }
      ...
      function getAvgSize() {
        ...
        for (let index = 0; index < children.length; index++) {
          ...
          const outerSize = getOuterSize(el); // 有问题吧因为padding
